...
int main() {
    ...
    std::vector<double> oxygenLevels;
    ...
    for(int i = 1; i <= trainingRounds; ++i) {
        ...
        double oxygenLevel = readDoubleInRange("Enter oxygen level (0-100): ", 0, 100);
         ...
         // Declare a vector to hold values of oxygen levels
        oxygenLevels.push_back(oxygenLevel);
    ...
    // Read oxygen levels from the vector and print out
    for(int i = 0; i < oxygenLevels.size(); ++i) {
        std::cout << "Oxygen level " << i << " = " << oxygenLevels[i] << std::endl;
    }

    for(std::size_t i = 0; i < batteryLevels.size(); ++i) {
        std::cout << "Battery level " << i << " = " << batteryLevels[i] << std::endl;
    }

    std::cout << "Oxygen levels:" << std::endl;

    for( double oxygenLvl : oxygenLevels) {
        std::cout << oxygenLvl << "; ";
    }
    std::cout << std::endl;

    // An exception will occur
    // std::cout << "Oxygen level " << oxygenLevels.size() << "= " << oxygenLevels[oxygenLevels.size()] << std::endl;
